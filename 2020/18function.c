 ...
 *
 * Compile:
 *     cc -std=c17 -Wall -Wextra -pedantic 18function.c
 * Enable timer:
 *     cc -O3 -march=native -mtune=native -DTIMER ../startstoptimer.c 18function.c
 * Test output with timer enabled:
 *     ./a.out | tail -n1
 * Get minimum runtime from timer output in bash:
 *     m=99999999;for((i=0;i<20000;++i));do t=$(./a.out 2>&1 1>/dev/null|awk '{print $2}');((t<m))&&m=$t&&echo "$m ($i)";done
 * Minimum runtime measurements:
 *     Macbook Pro 2024 (M4 4.4 GHz) :  21.9 µs
...
typedef void (*fun)(u64 *const a, const u64 x);
static void add(u64 *const a, const u64 x) { *a += x; }
static void mul(u64 *const a, const u64 x) { *a *= x; }
    ...
    for (const char *c = input; c != end; ++c)
        switch (*c) {  // cases ordered by value was fastest for me
            ...
            case '(':  // push
                stack1[len1++] = x1;
                x1 = init;
                stack2[len2++] = mark;  // first add marker (op=NULL)
                stack2[len2++] = x2;
                x2 = init;
                break;
            case ')':  // pop
