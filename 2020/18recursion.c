 ...
 *
 * Compile:
 *     cc -std=c17 -Wall -Wextra -pedantic 18recursion.c
 * Enable timer:
 *     cc -O3 -march=native -mtune=native -DTIMER ../startstoptimer.c 18recursion.c
...
static u64 sum2(void)
{
    ...
    for (;;)
        if (*ch >= '0') {
            ...
            case '*': ch += 2; return res * sum2();  // max recursion level: 10
            case '+': ch += 2; op = '+'; break;
            case ' ': ch++; break;
