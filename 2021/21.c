 ...
 *     Macbook Pro 2024 (M4 4.4 GHz) : 367 µs
...
static uint64_t cache[CACHE_SZ];  // cache of intermediate results
static bool incache[CACHE_SZ];
    ...
    const bool     player1)
{
    ...
    const uint32_t hash = ((((pos1 - 1)*10 + pos2 - 1)*21 + score1)*21 + score2)*2 + player1;
    if (incache[hash])
        return cache[hash];
